...
func BuildStart(c *gin.Context) {
	...
	packFile := gostring.JoinStrings(syncd.App.LocalTarSpace, "/", gostring.Int2Str(build.ApplyId), ".dockername")
	...
	bui, err := buiTask.NewBuild(repo, workSpace, syncd.App.LocalTmpSpace, packFile, p.BuildScript, build.ApplyId)
	if err != nil {
		render.AppError(c, err.Error())
		return
	}
